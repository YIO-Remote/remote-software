#ifndef SEARCHMODEL_MEDIAPLAYER_H
#define SEARCHMODEL_MEDIAPLAYER_H
...
public:
    ...
    SearchModelItem(const QString& type, QObject* model) :
        m_type(type),
        m_model(model)
    {}
...
public:
    ...
    Q_INVOKABLE QObject* returnSubModel(int index)
    {
...
{
public:
    SearchModelListItem(const QString& key, const QString& type, const QString& title, const QString& subtitle, const QString& imageUrl, const QVariant& commands) :
        m_key(key),
        m_type(type),
        m_title(title),
        m_subtitle(subtitle),
...
public:
    enum SearchRoles {
        ...
        TypeRole, TitleRole, SubTitleRole, ImageUrlRole, CommandsRole
    };
