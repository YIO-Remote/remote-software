...
Notifications::Notifications(QQmlApplicationEngine *engine) : m_engine(engine) {
    s_instance = this;
}
...
void Notifications::add(bool error, const QString &text, const QString &actionlabel, void (*f)(QObject *),
    ...
    qCDebug(lcNotification) << "Notification object created";

    // check if the same notification exists, then remove it
    for (QMap<int, Notification *>::const_iterator iter = m_notifications.cbegin(); iter != m_notifications.cend();
         ++iter) {
        if (iter.value()->m_text.contains(text) && m_notifications.contains(iter.key())) {
            qCDebug(lcNotification) << "Notification with same text found";
            m_notifications.remove(iter.key());
            qCDebug(lcNotification) << "Notification object removed" << iter.key();
            break;
        }
    }

    m_notifications.insert(m_id, n);
    qCInfo(lcNotification) << "New notification added" << n;

    emit listChanged();
    emit errorChanged();
    qCDebug(lcNotification) << "Signals emitted";

    show(m_id);
    qCDebug(lcNotification) << "Notification shown";

    m_id++;
}

void Notifications::add(bool error, const QString &text) {
    add(error, text, "", nullptr, nullptr);
}
...
void Notifications::execute(int id) {
    qCDebug(lcNotification) << "Executing function" << m_notifications.value(id)->m_text
                            << m_notifications.value(id)->m_actionLabel;
    qCDebug(lcNotification) << "Creating pointer";
    void (*f)(QObject *) = m_notifications.value(id)->m_action;
    qCDebug(lcNotification) << "Creating param";
    QObject *param = m_notifications.value(id)->m_param;
    qCDebug(lcNotification) << "Executing function";
    f(param);
}
